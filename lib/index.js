import { jsxs as S, jsx as Y } from "react/jsx-runtime";
import { useRef as b, useState as w, useEffect as h } from "react";
const F = (t, n, s, r) => {
  const o = t.getContext("2d");
  o && o.drawImage(n, 0, 0, s, r);
}, B = (t, n) => {
  const s = { x: 0, y: 0 }, r = { x: 0, y: 0 };
  if (n.offsetParent !== void 0 && n.offsetParent instanceof HTMLElement)
    do
      r.x += n.offsetLeft, r.y += n.offsetTop;
    while (n = n.offsetParent);
  return t instanceof TouchEvent ? (s.x = t.touches[0].clientX - r.x, s.y = t.touches[0].clientY - r.y) : (s.x = t.pageX - r.x, s.y = t.pageY - r.y), s;
}, P = (t, n) => Math.sqrt(Math.pow(n.x - t.x, 2) + Math.pow(n.y - t.y, 2)), z = (t, n) => Math.atan2(n.x - t.x, n.y - t.y), M = (t, n, s, r) => {
  const o = n.getContext("2d");
  if (o) {
    for (var a = o.getImageData(0, 0, s, r), A = a.data, f = A.length, d = f / t, c = 0, l = c = 0; l < f; l += t)
      parseInt(String(A[l])) === 0 && c++;
    return Math.round(c / d * 100);
  }
}, R = new Image();
R.src = "data:image/png;base64,";
const q = ({
  children: t,
  cover: n,
  onComplete: s,
  percent: r = 50,
  width: o = 320,
  height: a = 240
}) => {
  const A = b(document.createElement("canvas")), [f, d] = w(!1), [c, l] = w("none"), [I, J] = w(), m = b(!1), u = b({ x: 0, y: 0 }), E = (e) => {
    m.current = !1, e.removeEventListener("mousedown", g), e.removeEventListener("mousemove", y), e.removeEventListener("mouseup", C), e.removeEventListener("touchstart", g), e.removeEventListener("touchmove", y), e.removeEventListener("touchend", C);
  };
  h(() => {
    const e = new Image();
    e.onload = () => {
      l("block"), J(e), d(!0);
    }, e.src = n;
  }, []);
  const g = (e) => {
    m.current = !0, u.current = B(e, A.current);
  }, y = (e) => {
    if (!m.current) return;
    e.preventDefault?.();
    const Q = B(e, A.current), L = P(u.current, Q), V = z(u.current, Q), i = A.current, v = i.getContext("2d");
    Array.from(Array(Math.floor(L)).keys()).forEach((x) => {
      const { x: U, y: W } = u.current, Z = U + Math.sin(V) * x - 25, D = W + Math.cos(V) * x - 25;
      v && (v.globalCompositeOperation = "destination-out", v.drawImage(R, Z, D));
    }), u.current = Q;
    const p = M(32, i, i.width, i.height);
    p && p >= r && (d(!1), s?.(), E(i));
  }, C = () => {
    m.current = !1;
  };
  return h(() => {
    if (I) {
      const e = A.current;
      e && (F(e, I, o, a), e.addEventListener("mousedown", g, !1), e.addEventListener("mousemove", y, !1), e.addEventListener("mouseup", C, !1), e.addEventListener("touchstart", g, !1), e.addEventListener("touchmove", y, !1), e.addEventListener("touchend", C, !1));
    }
  }, [I]), /* @__PURE__ */ S("div", { className: "LescaScratchCard", style: { position: "relative", width: o, height: a, display: c }, children: [
    t,
    f && /* @__PURE__ */ Y(
      "canvas",
      {
        ref: A,
        width: o,
        height: a,
        style: { position: "absolute", backgroundColor: "transparent", top: 0, left: 0 }
      }
    )
  ] });
};
export {
  q as default
};
