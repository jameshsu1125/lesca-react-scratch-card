import { jsxs as M, jsx as z } from "react/jsx-runtime";
import { useRef as m, useState as w, useEffect as p } from "react";
const q = (n, e, A, r) => {
  const o = n.getContext("2d", { willReadFrequently: !0 });
  o && o.drawImage(e, 0, 0, A, r);
}, h = (n, e) => {
  const A = { x: 0, y: 0 }, r = { x: 0, y: 0 };
  if (e.offsetParent !== void 0 && e.offsetParent instanceof HTMLElement)
    do
      r.x += e.offsetLeft, r.y += e.offsetTop;
    while (e = e.offsetParent);
  return A.x = n.clientX - r.x, A.y = n.clientY - r.y, A;
}, K = (n, e) => Math.sqrt(Math.pow(e.x - n.x, 2) + Math.pow(e.y - n.y, 2)), j = (n, e) => Math.atan2(e.x - n.x, e.y - n.y), O = (n, e, A, r) => {
  const o = e.getContext("2d", { willReadFrequently: !0 });
  if (o) {
    for (var i = o.getImageData(0, 0, A, r), a = i.data, l = a.length, c = l / n, s = 0, u = s = 0; u < l; u += n)
      parseInt(String(a[u])) === 0 && s++;
    return Math.round(s / c * 100);
  }
}, U = new Image();
U.src = "data:image/png;base64,";
const X = ({
  children: n,
  cover: e,
  onComplete: A,
  percent: r = 50,
  width: o = 320,
  height: i = 240,
  brushSize: a = { width: 50, height: 50 },
  offset: l = { x: 0, y: 0 }
}) => {
  const c = m(document.createElement("canvas")), s = m(l), [u, x] = w(!1), [W, Z] = w("none"), [C, D] = w(), g = m(!1), f = m({ x: 0, y: 0 }), E = (t) => {
    g.current = !1, t.removeEventListener("pointerdown", R), t.removeEventListener("pointermove", V), t.removeEventListener("pointerup", v);
  };
  p(() => {
    s.current = l;
  }, [l]), p(() => {
    const t = new Image();
    t.onload = () => {
      Z("block"), D(t), x(!0);
    }, t.src = e;
  }, []);
  const R = (t) => {
    g.current = !0;
    const y = h(t, c.current);
    f.current = { x: y.x + s.current.x, y: y.y + s.current.y };
  }, V = (t) => {
    if (!g.current) return;
    t.preventDefault?.();
    const y = h(t, c.current), I = { x: y.x + s.current.x, y: y.y + s.current.y }, F = K(f.current, I), b = j(f.current, I), d = c.current, Q = d.getContext("2d", { willReadFrequently: !0 });
    Array.from(Array(Math.floor(F)).keys()).forEach((J) => {
      const { x: L, y: P } = f.current, Y = L + Math.sin(b) * J - a.width / 2, S = P + Math.cos(b) * J - a.height / 2;
      Q && (Q.globalCompositeOperation = "destination-out", Q.drawImage(U, Y, S, a.width, a.height));
    }), f.current = I;
    const B = O(32, d, d.width, d.height);
    B && B >= r && (x(!1), A?.(), E(d));
  }, v = () => {
    g.current = !1;
  };
  return p(() => {
    if (C) {
      const t = c.current;
      t && (q(t, C, o, i), t.addEventListener("pointerdown", R, !1), t.addEventListener("pointermove", V, !1), t.addEventListener("pointerup", v, !1));
    }
  }, [C]), /* @__PURE__ */ M("div", { className: "LescaScratchCard", style: { position: "relative", width: o, height: i, display: W }, children: [
    n,
    u && /* @__PURE__ */ z(
      "canvas",
      {
        ref: c,
        width: o,
        height: i,
        style: { position: "absolute", backgroundColor: "transparent", top: 0, left: 0 }
      }
    )
  ] });
};
export {
  X as default
};
