import { jsxs as Y, jsx as P } from "react/jsx-runtime";
import { useRef as v, useState as h, useEffect as x } from "react";
const S = (t, n, s, r) => {
  const o = t.getContext("2d", { willReadFrequently: !0 });
  o && o.drawImage(n, 0, 0, s, r);
}, b = (t, n) => {
  const s = { x: 0, y: 0 }, r = { x: 0, y: 0 };
  if (n.offsetParent !== void 0 && n.offsetParent instanceof HTMLElement)
    do
      r.x += n.offsetLeft, r.y += n.offsetTop;
    while (n = n.offsetParent);
  return t instanceof TouchEvent ? (s.x = t.touches[0].clientX - r.x, s.y = t.touches[0].clientY - r.y) : (s.x = t.pageX - r.x, s.y = t.pageY - r.y), s;
}, M = (t, n) => Math.sqrt(Math.pow(n.x - t.x, 2) + Math.pow(n.y - t.y, 2)), z = (t, n) => Math.atan2(n.x - t.x, n.y - t.y), q = (t, n, s, r) => {
  const o = n.getContext("2d", { willReadFrequently: !0 });
  if (o) {
    for (var l = o.getImageData(0, 0, s, r), a = l.data, A = a.length, C = A / t, c = 0, u = c = 0; u < A; u += t)
      parseInt(String(a[u])) === 0 && c++;
    return Math.round(c / C * 100);
  }
}, B = new Image();
B.src = "data:image/png;base64,";
const O = ({
  children: t,
  cover: n,
  onComplete: s,
  percent: r = 50,
  width: o = 320,
  height: l = 240,
  brushSize: a = { width: 50, height: 50 }
}) => {
  const A = v(document.createElement("canvas")), [C, c] = h(!1), [u, J] = h("none"), [I, E] = h(), d = v(!1), i = v({ x: 0, y: 0 }), L = (e) => {
    d.current = !1, e.removeEventListener("mousedown", m), e.removeEventListener("mousemove", g), e.removeEventListener("mouseup", y), e.removeEventListener("touchstart", m), e.removeEventListener("touchmove", g), e.removeEventListener("touchend", y);
  };
  x(() => {
    const e = new Image();
    e.onload = () => {
      J("block"), E(e), c(!0);
    }, e.src = n;
  }, []);
  const m = (e) => {
    d.current = !0, i.current = b(e, A.current);
  }, g = (e) => {
    if (!d.current) return;
    e.preventDefault?.();
    const Q = b(e, A.current), U = M(i.current, Q), R = z(i.current, Q), f = A.current, w = f.getContext("2d", { willReadFrequently: !0 });
    Array.from(Array(Math.floor(U)).keys()).forEach((p) => {
      const { x: W, y: Z } = i.current, D = W + Math.sin(R) * p - a.width / 2, F = Z + Math.cos(R) * p - a.height / 2;
      w && (w.globalCompositeOperation = "destination-out", w.drawImage(B, D, F, a.width, a.height));
    }), i.current = Q;
    const V = q(32, f, f.width, f.height);
    V && V >= r && (c(!1), s?.(), L(f));
  }, y = () => {
    d.current = !1;
  };
  return x(() => {
    if (I) {
      const e = A.current;
      e && (S(e, I, o, l), e.addEventListener("mousedown", m, !1), e.addEventListener("mousemove", g, !1), e.addEventListener("mouseup", y, !1), e.addEventListener("touchstart", m, !1), e.addEventListener("touchmove", g, !1), e.addEventListener("touchend", y, !1));
    }
  }, [I]), /* @__PURE__ */ Y("div", { className: "LescaScratchCard", style: { position: "relative", width: o, height: l, display: u }, children: [
    t,
    C && /* @__PURE__ */ P(
      "canvas",
      {
        ref: A,
        width: o,
        height: l,
        style: { position: "absolute", backgroundColor: "transparent", top: 0, left: 0 }
      }
    )
  ] });
};
export {
  O as default
};
