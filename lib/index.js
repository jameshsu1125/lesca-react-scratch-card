import { jsxs as S, jsx as M } from "react/jsx-runtime";
import { useRef as v, useState as h, useEffect as b } from "react";
const z = (t, n, s, r) => {
  const o = t.getContext("2d", { willReadFrequently: !0 });
  o && o.drawImage(n, 0, 0, s, r);
}, B = (t, n) => {
  const s = { x: 0, y: 0 }, r = { x: 0, y: 0 };
  if (n.offsetParent !== void 0 && n.offsetParent instanceof HTMLElement)
    do
      r.x += n.offsetLeft, r.y += n.offsetTop;
    while (n = n.offsetParent);
  return t instanceof TouchEvent ? (s.x = t.touches[0].clientX - r.x, s.y = t.touches[0].clientY - r.y) : (s.x = t.pageX - r.x, s.y = t.pageY - r.y), s;
}, q = (t, n) => Math.sqrt(Math.pow(n.x - t.x, 2) + Math.pow(n.y - t.y, 2)), K = (t, n) => Math.atan2(n.x - t.x, n.y - t.y), j = (t, n, s, r) => {
  const o = n.getContext("2d", { willReadFrequently: !0 });
  if (o) {
    for (var i = o.getImageData(0, 0, s, r), A = i.data, a = A.length, c = a / t, u = 0, l = u = 0; l < a; l += t)
      parseInt(String(A[l])) === 0 && u++;
    return Math.round(u / c * 100);
  }
}, J = new Image();
J.src = "data:image/png;base64,";
const H = ({
  children: t,
  cover: n,
  onComplete: s,
  percent: r = 50,
  width: o = 320,
  height: i = 240,
  brushSize: A = { width: 50, height: 50 },
  offset: a = { x: 0, y: 0 }
}) => {
  const c = v(document.createElement("canvas")), [u, l] = h(!1), [E, L] = h("none"), [Q, U] = h(), m = v(!1), d = v({ x: 0, y: 0 }), W = (e) => {
    m.current = !1, e.removeEventListener("mousedown", g), e.removeEventListener("mousemove", C), e.removeEventListener("mouseup", I), e.removeEventListener("touchstart", g), e.removeEventListener("touchmove", C), e.removeEventListener("touchend", I);
  };
  b(() => {
    const e = new Image();
    e.onload = () => {
      L("block"), U(e), l(!0);
    }, e.src = n;
  }, []);
  const g = (e) => {
    m.current = !0;
    const y = B(e, c.current);
    d.current = { x: y.x + a.x, y: y.y + a.y };
  }, C = (e) => {
    if (!m.current) return;
    e.preventDefault?.();
    const y = B(e, c.current), w = { x: y.x + a.x, y: y.y + a.y }, Z = q(d.current, w), p = K(d.current, w), f = c.current, x = f.getContext("2d", { willReadFrequently: !0 });
    Array.from(Array(Math.floor(Z)).keys()).forEach((V) => {
      const { x: D, y: F } = d.current, Y = D + Math.sin(p) * V - A.width / 2, P = F + Math.cos(p) * V - A.height / 2;
      x && (x.globalCompositeOperation = "destination-out", x.drawImage(J, Y, P, A.width, A.height));
    }), d.current = w;
    const R = j(32, f, f.width, f.height);
    R && R >= r && (l(!1), s?.(), W(f));
  }, I = () => {
    m.current = !1;
  };
  return b(() => {
    if (Q) {
      const e = c.current;
      e && (z(e, Q, o, i), e.addEventListener("mousedown", g, !1), e.addEventListener("mousemove", C, !1), e.addEventListener("mouseup", I, !1), e.addEventListener("touchstart", g, !1), e.addEventListener("touchmove", C, !1), e.addEventListener("touchend", I, !1));
    }
  }, [Q]), /* @__PURE__ */ S("div", { className: "LescaScratchCard", style: { position: "relative", width: o, height: i, display: E }, children: [
    t,
    u && /* @__PURE__ */ M(
      "canvas",
      {
        ref: c,
        width: o,
        height: i,
        style: { position: "absolute", backgroundColor: "transparent", top: 0, left: 0 }
      }
    )
  ] });
};
export {
  H as default
};
