import { jsxs as M, jsx as z } from "react/jsx-runtime";
import { useRef as Q, useState as h, useEffect as p } from "react";
const q = (t, n, s, r) => {
  const o = t.getContext("2d", { willReadFrequently: !0 });
  o && o.drawImage(n, 0, 0, s, r);
}, J = (t, n) => {
  const s = { x: 0, y: 0 }, r = { x: 0, y: 0 };
  if (n.offsetParent !== void 0 && n.offsetParent instanceof HTMLElement)
    do
      r.x += n.offsetLeft, r.y += n.offsetTop;
    while (n = n.offsetParent);
  return t instanceof TouchEvent ? (s.x = t.touches[0].clientX - r.x, s.y = t.touches[0].clientY - r.y) : (s.x = t.pageX - r.x, s.y = t.pageY - r.y), s;
}, K = (t, n) => Math.sqrt(Math.pow(n.x - t.x, 2) + Math.pow(n.y - t.y, 2)), j = (t, n) => Math.atan2(n.x - t.x, n.y - t.y), O = (t, n, s, r) => {
  const o = n.getContext("2d", { willReadFrequently: !0 });
  if (o) {
    for (var u = o.getImageData(0, 0, s, r), c = u.data, l = c.length, a = l / t, A = 0, i = A = 0; i < l; i += t)
      parseInt(String(c[i])) === 0 && A++;
    return Math.round(A / a * 100);
  }
}, E = new Image();
E.src = "data:image/png;base64,";
const X = ({
  children: t,
  cover: n,
  onComplete: s,
  percent: r = 50,
  width: o = 320,
  height: u = 240,
  brushSize: c = { width: 50, height: 50 },
  offset: l = { x: 0, y: 0 }
}) => {
  const a = Q(document.createElement("canvas")), A = Q(l), [i, R] = h(!1), [L, U] = h("none"), [w, W] = h(), m = Q(!1), d = Q({ x: 0, y: 0 }), Z = (e) => {
    m.current = !1, e.removeEventListener("mousedown", g), e.removeEventListener("mousemove", C), e.removeEventListener("mouseup", I), e.removeEventListener("touchstart", g), e.removeEventListener("touchmove", C), e.removeEventListener("touchend", I);
  };
  p(() => {
    A.current = l;
  }, [l]), p(() => {
    const e = new Image();
    e.onload = () => {
      U("block"), W(e), R(!0);
    }, e.src = n;
  }, []);
  const g = (e) => {
    m.current = !0;
    const f = J(e, a.current);
    d.current = { x: f.x + A.current.x, y: f.y + A.current.y };
  }, C = (e) => {
    if (!m.current) return;
    e.preventDefault?.();
    const f = J(e, a.current), x = { x: f.x + A.current.x, y: f.y + A.current.y }, D = K(d.current, x), V = j(d.current, x), y = a.current, v = y.getContext("2d", { willReadFrequently: !0 });
    Array.from(Array(Math.floor(D)).keys()).forEach((B) => {
      const { x: F, y: Y } = d.current, P = F + Math.sin(V) * B - c.width / 2, S = Y + Math.cos(V) * B - c.height / 2;
      v && (v.globalCompositeOperation = "destination-out", v.drawImage(E, P, S, c.width, c.height));
    }), d.current = x;
    const b = O(32, y, y.width, y.height);
    b && b >= r && (R(!1), s?.(), Z(y));
  }, I = () => {
    m.current = !1;
  };
  return p(() => {
    if (w) {
      const e = a.current;
      e && (q(e, w, o, u), e.addEventListener("mousedown", g, !1), e.addEventListener("mousemove", C, !1), e.addEventListener("mouseup", I, !1), e.addEventListener("touchstart", g, !1), e.addEventListener("touchmove", C, !1), e.addEventListener("touchend", I, !1));
    }
  }, [w]), /* @__PURE__ */ M("div", { className: "LescaScratchCard", style: { position: "relative", width: o, height: u, display: L }, children: [
    t,
    i && /* @__PURE__ */ z(
      "canvas",
      {
        ref: a,
        width: o,
        height: u,
        style: { position: "absolute", backgroundColor: "transparent", top: 0, left: 0 }
      }
    )
  ] });
};
export {
  X as default
};
